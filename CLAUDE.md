...
- **Voice**: Technical but accessible — assume the reader knows programming but not Petri nets. Before publishing a chapter, run the `voice-audit` skill (`../blog-stackdump-com/.claude/skills/voice-audit/SKILL.md`) — it lists the LLM-draft tells and the argument defects (B-checks) this material is prone to.
